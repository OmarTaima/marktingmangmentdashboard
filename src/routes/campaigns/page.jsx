...
const CampaignsPage = () => {
    ...
    // overlay removed — transitions should be instant
    ...
    const handleTrackClick = (clientId) => {
        if (isTransitioningRef.current) return;
        isTransitioningRef.current = true;

        // show overlay and fade in
        setOverlayVisible(true);
        // small tick to allow DOM update
        setTimeout(() => setOverlayFadeIn(true), 10);

        // once overlay is opaque, set selected client and persist (shorter)
        setTimeout(() => {
            try {
        ...
        // fade overlay out sooner
        setTimeout(() => setOverlayFadeIn(false), 380);

        // hide overlay and allow future transitions
        setTimeout(() => {
    ...
    return (
        <div className="relative space-y-6 md:space-y-8">
            ...
            {/* loaders removed per request - overlay handles transition */}

            {!selectedClientId && !isResetting && !isLoading && !isTransitioning && (
                <div>
                    {clients.length > 0 ? (
                        <>
                            <h2 className="text-secondary-900 dark:text-secondary-50 mb-4 text-lg font-semibold">{t("select_a_client_to_plan")}</h2>
                            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                                {clients.map((client) => {
                                    ...
                                    return (
                                        <div
                                            key={client.id}
                                            className="card hover:border-primary-500 flex h-full flex-col transition-all"
                                        >
                                            ...
                                            {plan ? (
                                                ...
                                                <div className="mt-3 text-xs text-yellow-600 dark:text-yellow-400">⚠️ {t("no_plan_created_yet")}</div>
                                            )}
                                            <button
                                                onClick={() => handleTrackClick(client.id)}
                        ...
                        <div className="card bg-primary-50 dark:bg-primary-950/30">
                            ...
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-secondary-900 dark:text-secondary-50 text-lg font-bold">{planData.timeline || "N/A"}</p>
                                </div>
                                <div>
                                        ...
                                        >
                                            ...
                                            <div className="flex-1">
                                                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                                                    <div>
                                                        <h4 className="text-secondary-900 dark:text-secondary-50 font-medium">{upload.title}</h4>
            ...
            {/* Overlay used to fade between lists and client details. */}
            {overlayVisible && (
                <div

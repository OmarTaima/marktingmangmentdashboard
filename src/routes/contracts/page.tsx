...
لا تتجاوز مسؤولية الوكالة إجمالي المبلغ المدفوع بموجب هذه الاتفاقية.`;
        ...
        const fillTemplate = (template: string, opts: { client?: any; plan?: any; pkg?: any } = {}) => {
            const { client = null, plan = null, pkg = null } = opts;
            const now = new Date();
            const dateStr = now.toLocaleDateString(String(lang) === "ar" ? "ar-EG" : undefined);
                                        ...
                                        {String(lang) === "ar" ? (
                                            <>
                                                <input
                                                    ...
                                                    dir="rtl"
                                                    className={`border-light-600 text-light-900 focus:border-light-500 dark:border-dark-700 dark:bg-dark-800 dark:text-dark-50 placeholder:text-light-600 dark:placeholder:text-dark-400 flex-1 rounded-lg border px-3 py-2 text-sm transition-colors duration-300 focus:outline-none ${String(lang) === "ar" ? "text-right" : "text-left"}`}
                                                />
                                                <input
                                                    dir="ltr"
                                                    placeholder={
                                                    ...
                                                    className={`border-light-600 text-light-900 focus:border-light-500 dark:border-dark-700 dark:bg-dark-800 dark:text-dark-50 placeholder:text-light-600 dark:placeholder:text-dark-400 flex-1 rounded-lg border px-3 py-2 text-sm transition-colors duration-300 focus:outline-none ${String(lang) === "en" ? "text-left" : "text-right"}`}
                                                />
                                            </>
                                        ) : (
                                            <>
